...
class Tip(pygame.sprite.Sprite):
    ...
    def __init__(self, type, group, pos=None, camp=None, font=None, text="", cards=None, index=0, color=(255, 255, 255)):
        ...
        if self.type == "decoration_2":
            ...
            self.rect = self.image.get_frect(topleft = pos)
        if self.type == "ll":
            self.image = pygame.image.load(join(BASEPATH, "images", "landlord.png")).convert()
...
class Engine:
    ...
    def initialize(self):
        ...
        self.suits = {
        '♠': pygame.image.load(join(BASEPATH, "images", "spade.png")).convert_alpha(), 
        '♥': pygame.image.load(join(BASEPATH, "images", "heart.png")).convert_alpha(), 
        '♣': pygame.image.load(join(BASEPATH, "images", "club.png")).convert_alpha(), 
        '♦': pygame.image.load(join(BASEPATH, "images", "diamond.png")).convert_alpha(), 
        'SJ': [], 
        'BJ': []
        }
